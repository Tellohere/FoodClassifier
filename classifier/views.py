...
def upload_image(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if predictions:
                pred = predictions[0]
                # Use top1 and top1conf for classification
                cls_id = pred.probs.top1
                confidence = pred.probs.top1conf.item()  # convert tensor to float
                class_name = CLASS_NAMES[cls_id]
                result = f"{class_name} ({confidence*100:.2f}%)"
    else:
        form = ImageUploadForm()

    return render(request, 'classifier/upload.html', {'form': form, 'result': result})
